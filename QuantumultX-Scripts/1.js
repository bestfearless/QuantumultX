...
// 主机名处理
function mergeHostnames(script) {
    const hostnameRegex = /hostname\s*=\s*([a-zA-Z0-9.-]+)/g;
    let hostnames = new Set();
    let match;
       
    while ((match = hostnameRegex.exec(script)) !== null) {
        hostnames.add(match[1]);
    }
       
    return `hostname=${Array.from(hostnames).join(',')}`;
}
// 在解析器的合适位置调用该函数
const mergedHostnames = mergeHostnames(scriptContent);
